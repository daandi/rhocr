...
describe OCRXWord do
  ...
  describe '#to_s' do
      it 'should print the coordinates of the box and the textual information' do
        p @ocrx_word
        @ocrx_word.to_s.should == "_test_:\tOCRXWord(10/15,20/20)"
      end
  end
  
  describe '#to_html(css_class)' do
      it 'should create an span elment to overlay an image on an html-page' do
        @ocrx_word.to_html.should == "<span style='position:absolute; top:15px; left:10px; height:5px; width:10px;' class='ocrx_word'><span class='word'>_test_</span></span>"
      end
